package com.example.administrator.mycamera.utils;

/**
 * Created by Administrator on 2018/6/26.
 */

public class CameraConstant {
    public static final int OPEN_CAMERA_PHOTO = 1001;
    public static final int SUCCESS_UPDATE_IMAGE_ToDb = 1002;
    public static final int FOCUS_SUCCESS = 1003;

    public static final int PHOTO_MODEL = 1004;
    public static final int VIDEO_MODEL = 1005;
    public static final int BEAUTY_MODEL = 1006;
    public static final int PANORAMIC_MODEL = 1007;
    public static final int VIDEO_START = 1008;


    public static final int COUNT_DOWN_O = 0;
    public static final int COUNT_DOWN_2 = 2;
    public static final int COUNT_DOWN_5 = 5;
    public static final int COUNT_DOWN_10 = 10;

    public static final String COUNT_DOWN_DATA="count_down_data";
    public static final int COUNT_DOWN_TAKE_PHOTO = 1009;
    public static final String SETTING_FRAGMENT = "setting_fragment";

    public static final int START_RECORDING = 1012;
    public static final int STOP_RECORDING = 1013;




}
